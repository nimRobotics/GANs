...
import numpy as np
import ot # include POT for optimal transport
import torch
...
def compute_score(real, fake, k=1, sigma=1, sqrt=True):
    ...
    return wasserstein(Mxy, sqrt), mmd(Mxx, Mxy, Myy, sigma), knn(Mxx, Mxy, Myy, k, sqrt).recall, knn(Mxx, Mxy, Myy, k, sqrt).precision, knn(Mxx, Mxy, Myy, k, sqrt).acc_t, knn(Mxx, Mxy, Myy, k, sqrt).acc_f, knn(Mxx, Mxy, Myy, k, sqrt).acc
